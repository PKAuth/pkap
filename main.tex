...
%, the secret key's identifier matches the identifier in the \texttt{PKAMac}
...
%The key \texttt{identifier} is the unique identifier of the secret key used to encrypt the content. 
...
% and the key's identifier matches the identifier in the \texttt{PKASymEncrypted a}. 
...
% The public key identifier (\texttt{PKAIdentifier})
...
% , the key's identifier matches the identifier in the \texttt{PKAMac}, 
...
On any failure, the service MUST return a \texttt{400 Bad Request} response with content of a JSON encoded \texttt{PKAError} (\Cref{code:pkaerror}). 
...
% 6 & Invalid public key (no cryptographic identity for user or supplied public key is not in the cryptographic identity) \\
7 & Challenge expired \\
...
\end{figure}
% JP: Cheating with the URL type



The definition of type \texttt{PKAResponse} is shown in \Cref{code:pkaresponse}. 
...
An example with a redirect field is shown in \Cref{fig:logout_redirect}. 
...
The service MAY return content of type \texttt{PKAResponse} (\Cref{code:pkaresponse}). 
% indicate a browser redirect to the client over AJAX by returning JSON as shown in \Cref{fig:logout_redirect}. 
...
On any failure, the service MAY return a \texttt{400 Bad Request} response with content of a JSON encoded \texttt{PKAError} (\Cref{code:pkaerror}). 
