...
Section \ref{sec:algorithms} defines a standardized encoding format for the cryptographic algorithms used by \pkap{}. 
Section \ref{sec:identities} defines how cryptographic identities are encoded. 
Section \ref{sec:pkap} details how \pkap{} clients and servers interact to authenticate users and share cryptographic identities. 
...
The only currently valid strings are \texttt{"admin"}, \texttt{"auth"}, \texttt{"write"}, and \texttt{"read"}. 
...
In particular, \pkap{} compliant websites MUST embed the html header tag \texttt{pkauth}. 
